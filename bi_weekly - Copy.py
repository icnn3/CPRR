...
def load_totaldefects_data():
    ...
    with Spinner():
        ...
        vuln['Compliance Requirement'] = vuln['Compliance Requirement'].astype('datetime64')

        vuln['Exception Expiration Date'] = vuln['Exception Expiration Date'].fillna("")# replacing NaN values
        vuln['Exception Expiration Date'] = vuln['Exception Expiration Date'].astype('datetime64')

        vuln['Open Time'] = vuln['Open Time'].astype('datetime64')
        vuln['Close Time'] = vuln['Close Time'].astype('datetime64')
        vuln['MaxTimeToFix'] = vuln['MaxTimeToFix'].astype('datetime64')
...
def load_exception_data():
    ...
    exceptions['Expiration Date'] = exceptions['Expiration Date'].astype('datetime64')
...
def load_servicenow_data():
    ...
    with Spinner():
        ...
        serviceNow['Installation Start D/T'] = serviceNow['Installation Start D/T'].astype('datetime64')
        serviceNow['Installation End D/T'] = serviceNow['Installation End D/T'].astype('datetime64')
...
def extract_chg_dataframe(vuln_data):
    ...
    l_closed['Upcoming CHG Installation Start D/T'] = l_closed['Upcoming CHG Installation Start D/T'].astype('datetime64')
    ...
    l_closed['Upcoming CHG Installation End D/T'] = l_closed['Upcoming CHG Installation End D/T'].fillna(
        "")  # replacing NaN values
    l_closed['Upcoming CHG Installation End D/T'] = l_closed['Upcoming CHG Installation End D/T'].astype('datetime64')
